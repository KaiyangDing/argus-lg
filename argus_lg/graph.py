...
v0.1 → v0.2（reports/v02_devlog.md 记录全过程）：
- researcher 从单轮流水线升级为多轮 map-reduce agent：每轮 检索→备忘录消化→缺口自评，
  总证据无上限（全语料可及），单次调用证据窗有界（撞墙⑤实测边界：单窗过大引用纪律崩塌）；
- supervisor 产出带 key_questions 的研究框架（五维尽调基线）；merge 后复审补派一轮；
- write 三层化：投资要点 / 各节三段式（事实→分析→风险）/ 跨方面关联 / 风险汇总 / 边界；
  备忘录内〔chunk_id〕引用由代码映射为全局 [n]——编号翻译不交给 LLM。
防幻觉三闸不动：零证据不调 LLM、事实句末引用+评测 quote 在场校验、陷阱金标每轮跑。
"""
...
class Finding(TypedDict):
    ...
    evidence: list[EvidenceRef]  # 核心证据（researcher 自评精选）
...
class RetryingStruct:
    """结构化输出双坑的工程解（v02_devlog）：json_schema=三层嵌套 schema 服务端挂死
    （确定性）→ 改走 function_calling；function_calling=模型偶发无视强制 tool_choice、
    直接文字作答 → 解析 None（概率性，温度 0 不免疫）→ 空返回重试 ≤3，仍空报错快速失败。"""

    def __init__(self, chat: BaseChatModel, model_cls: type[BaseModel], attempts: int = 3) -> None:
        self._model_cls = model_cls
        self._attempts = attempts
        self._runnable = chat.with_structured_output(model_cls, method="function_calling")
    ...
    def invoke(self, msgs: object) -> BaseModel:
        ...
        for attempt in range(self._attempts):
            ...
            if out is not None:
                return out
            # 拒调是粘性的（szss 实测同输入三连拒）：注入催办消息打破同输入循环
...
def merge_findings(findings: list[Finding]) -> tuple[list[EvidenceRef], list[Section]]:
    ...
    同名方面归并为一节（iteration-1 病：复审补研与原方面同名 → 章节重复且互相矛盾）：
    补研备忘录以【补充研究】拼入原节，证据并集。
    """
...
def render_evidence_subset(evidence: Sequence[EvidenceRef], refs: Sequence[int]) -> str:
    return "\n".join(
        f"[{n}] ({_loc(evidence[n - 1])}) {evidence[n - 1]['text']}" for n in sorted(set(refs))
    )
...
def build_graph(
    ...
    （产品侧换成从其 Document 表生成同款概况，图零改动）。"""
    writer = write_chat or chat
    default_profile = "（未提供语料概况：年份与文档类型未知，先以宽泛查询探明时间范围。）"
    ...
    def supervisor(state: ResearchState) -> dict[str, object]:
        ...
        msgs = SUPERVISOR_PROMPT.invoke({"company": state["company"], "corpus_profile": profile})
    ...
    def researcher(state: ResearcherInput) -> dict[str, object]:
        ...
        for rnd in range(MAX_ROUNDS):
            ...
            core_ids = [cid for cid in refl.core_chunk_ids if cid in evidence_map][:CORE_CAP]
    ...
    def review(state: ResearchState) -> dict[str, object]:
        ...
        questions = "\n\n".join(
            f"### {a['name']}\n{render_questions(a['key_questions'])}" for a in state["aspects"]
        )
    ...
    def write(state: ResearchState) -> dict[str, object]:
        ...
        # 一致性核对 pass（iteration-1 病：并行研究员对同一指标给出矛盾数字无人调停）
        memos_mapped = "\n\n".join(
            f"### {s['name']}\n{map_memo_refs(s['summary'], numbers)}" for s in state["sections"]
        )
        conflicts = str(
            writer.invoke(
                CONSISTENCY_PROMPT.invoke({"company": state["company"], "memos": memos_mapped})
            ).content
        )
    ...
    # 节点级重试（iteration-3 事故：researcher 单次 API 超时三连杀死整跑）——
    # langgraph 内置 RetryPolicy = v1 自研重试组件的框架替代，LLM 节点全挂
