...
> **v0.2（2026-08-17~18，已收口）**：用户令「产出媲美华尔街研报的报告，像研究员一样迭代并全程记录」。
> 协议升级：材料评审为质量主仪器、金标降为防幻觉底线闸门、¥10 授权自主迭代。
> 终局：报告几百字→6000~9000 字三层研报；闸门 覆盖 **13/18**（历史最高）/陷阱 0×3/越界 0/忠实 15/18；
> 工程问题账 10 条闭环；天花板=表列对齐（mineru-api 契约获实证）；实花 ≈¥8.0。
> **全记录：reports/v02_devlog.md**（本文件以下为 v0.1 阶段存档）。
...
4. **2026-08-17 S5 一轮**：①**时间盲区**——yh 0/6 机检=报告通篇 2025、金标 2024 事实不在文本（检索层 S3 已证可及，病灶在 LLM 查询生成无年份维度）；处方一行（查询强制 2024/2025 各至少一条）→ 二轮 yh 0→3、总分 7→11 兑现。②**writer 幻觉+装饰性引用**——「宁波汉意被监管警示 [17][18]」全语料无此事实、被引块=退市样板页（预训练记忆掏事实+字面配引用）；二轮消失=非确定性，指标每轮必跑的理由。**【2026-08-17 深夜更正】v0.2 机检推翻②原判：关键词全文检索证实 lpz-ar-2024 p63 / lpz-ar-2025 p67 两块深处确含「湖北证监局警示函/通报批评/诚信档案」，[17][18] 恰指这两块——很可能是正确引用被误杀。原判系验证脚本只读块前 180 字预览（恰为退市样板文头）所致。撞墙④改判为：验证器自身的切片预览偏差——在场判定必须全文匹配，不能看预览。**
